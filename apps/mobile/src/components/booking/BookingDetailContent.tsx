...
// The booking-request detail CONTENT, extracted from app/bookings/[id].tsx
// (ME-15) so it renders in two hosts with one source of truth:
//   - the pushed /bookings/[id] route (compact/medium, unchanged behavior)
//   - the Requests list's detail pane at the expanded window class
// It is a pure function of `id` and owns its own fetch; TanStack Query keys by
// path, so the pane and the route share one cache entry (no double fetch when
// the window crosses the class boundary). Loading/error branches double as the
// pane's states.
//
// Round-12 detail rebuild (client feedback: the flat label/value list read small
// and empty). The submitted form info lives in bordered cards with icon
...
export function BookingDetailContent({ id }: { id: string }) {
  const router = useRouter();
  const c = useColors();
  // 0 when pushed as a route (native header owns the top); the pinned
  // bookings band's height when rendered inside the Requests detail pane.
