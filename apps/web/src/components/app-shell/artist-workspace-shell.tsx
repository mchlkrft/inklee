...
// The signed-in workspace chrome (sidebar, top bars, bottom nav, books pill,
// unread badge), extracted from the (artist) layout so the auth-OPTIONAL
// (map) group can render the identical chrome for signed-in artists without
// duplicating any of it (one source of truth; go-live plan S2). The caller
// owns auth: this component assumes a verified signed-in user and never
// redirects.
export default async function ArtistWorkspaceShell({
  userId,
  children,
}: {
  userId: string;
  ...
  void touchArtistActivity(userId, "web");
