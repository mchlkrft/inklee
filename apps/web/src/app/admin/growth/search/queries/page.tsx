...
// the acquisition tabs and are a different measurement.
...
export default async function SearchQueriesPage({
  ...
  void writeAudit({
    ...
    actor: adminId,
    category: "admin",
    details: { section: "search-queries" },
  });
  ...
  const [current, previous] = await Promise.all([
    gscDimensionAgg(propertyId, "query", window.fromDay, window.toDay, 250),
    gscDimensionAgg(
      propertyId,
      "query",
  ...
  const exportQuery = new URLSearchParams({ view: "queries" });
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <SectionHeading>Google queries</SectionHeading>
          <a
        ...
        <p className="text-xs text-muted-foreground">
          Search Console (delayed, source dates). Top 250 queries by
          impressions, {window.fromDay} to {window.toDay}, compared with{" "}
        ...
        {current.length > 0 ? (
          <>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              means the query moved up. Google clicks are not first-party
              visits; the acquisition tabs measure those separately.
            </p>
          </>
        ) : (
          <EmptyState text="No query rows synced for this window yet." />
        )}
      </section>
    </div>
  );
