...
**Positioning.** Inklee is the cleanest way for tattoo artists to turn Instagram attention into organized booking requests. Not an all-in-one studio OS, not a client-facing marketplace, not a generic scheduler. (Updated 2026-07-27, superseding the 2026-07-17 "artist-facing discovery" scoping: the Inklee 2.0 tattoo map is becoming a PUBLIC discovery layer per the founder's 2026-07-22 decision, defined in `docs/product/inklee-2-map-growth-strategy.md` as "find the place, build the connection, manage the work". The map is an acquisition and product-utility layer whose conversion is account creation through claims, watches, and guest spot actions. The marketplace guardrails stay intact: no client self-booking through the map, no client accounts, and clients still come from the artist's own audience for bookings.)
...
**Updated 2026-07-28 (the full-package directive; product SoT `docs/product/plus-product-spec.md`):** Plus is now a confirmed PACKAGE, not just polish + deposits: Inklee page with four Linkhub templates + shared appearance system, booking-form customization + a Plus-only large-project mode, Linkhub analytics (Free gets none), goods selling for ALL tiers (Free 3 / Plus 25 active products) with Plus goods tools (variants, inventory, preorders, scheduled drops, discounts, bundles, collections), and **two tier-differentiated fee lanes: deposits Free 3% / Plus 0.5% (supersedes the flat-3% OQ-7 note above where they conflict) and goods Free 5% / Plus 1%** on the subtotal after discounts ex VAT and shipping, with a Plus fee-savings dashboard as the measurable proof of value. Launch posture: full package only, no coming-soon. **Open economics flag (founder to resolve): a Free deposit rate implies Free card-deposit collection, which collides with the D-d Plus-gating argument this section still carries.** Positioning: "Your tattoo business, your way"; no brand-speak in customer copy.
...
>
> **Map growth addendum, 2026-07-27** (from `docs/product/inklee-2-map-growth-strategy.md`; direction only, no pricing decided). The public map changes this phase's business context in four ways. (1) **Acquisition:** the map becomes a public acquisition layer whose loops (studio claiming, guest spot opportunities, watch alerts, community corrections, city activation) all convert to account creation; the studio claim funnel is the primary studio-owner acquisition path. (2) **Studio owner value:** the paid role's value grows with the map (claimed profile, cockpit, guest spot inbox, signals, and later opportunity and analytics features), which strengthens the Q8 case without deciding its timing. (3) **Free versus paid boundary:** basic public discovery is never paywalled; registration unlocks continuity and professional action; potential paid value is limited to advanced operations (alerts, analytics, team tools, demand insight) and stays open until Q8/Q27 business-model work. (4) **Paid visibility:** if promotional placement ever exists it must be visibly labeled, separate from organic relevance, volume-limited, restricted to claimed complete profiles, and never disguised as a recommendation; selling placement before organic relevance works is explicitly rejected. None of this changes the pricing tables in this document.
