...
| `rich_content_blocks` | Gate the Hub's rich content blocks to Plus: image galleries today, and the home for future rich sections (video, testimonials) as they ship (founder ruling FD1, 2026-08-01, SUPERSEDES the earlier `appearance_custom` gate: a gallery is content, not styling, so it no longer rides the styling key) | No rich content blocks for anyone, which is exactly today's look before this key existed. Existing gallery blocks stay in `settings` and are hidden ONLY at render (D2); an unrelated Hub edit never strips one | `richContentBlocksAllowed`; the hub RENDER (`app/[slug]/hub/page.tsx`) and BOTH save paths (`saveBioPageAction` + `uploadGalleryImageAction` web, `POST /api/mobile/settings/hub` native) via the shared `gateMediaBlocksForSave`. ⚠️ Never granted to the legacy_free_v1 cohort (verified: `computeLegacyFreeV1Grant` only ever sets `custom_templates`), so no grant migration was needed for the split. A fresh EAS build is a prerequisite before granting to anyone on mobile (same gate as `goods_collections`/`goods_bundles`) |
