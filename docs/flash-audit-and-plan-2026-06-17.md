...
- **Instagram import — OPEN, blocked on Meta (2026-06-17):** the existing "Inklee" Meta app (Facebook Login for Business) cannot host Instagram Business Login (Meta's use-case restriction — the Instagram-login use case isn't offerable on that app), so a NEW Meta app is being created with the "Messaging und Content auf Instagram verwalten" use case using the **Instagram-Login** method (matches the code: `instagram.com/oauth/authorize` + `instagram_business_basic` + `graph.instagram.com`). **Blocked ~24h on Business-portfolio verification.** When verified: set the redirect URI `https://inklee.app/api/instagram/callback`, copy the Instagram app id/secret into Vercel prod env (`INSTAGRAM_APP_ID` / `INSTAGRAM_APP_SECRET` / `INSTAGRAM_STATE_SECRET`), redeploy → the **web** Instagram import works immediately. The **native** in-app-browser import entry (D4) ships after that (IG requires HTTPS redirects, so native reuses the same web callback in an in-app browser — no custom scheme). Code is ready; this is purely the external Meta + env step.
