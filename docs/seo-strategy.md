# Inklee SEO Strategy and Roadmap (analytical companion)

> **Shared and transparent for both ChatGPT and Claude Code.** As of 2026-07-02 the **canonical** SEO source of truth is [`docs/seo/inklee-seo-strategy.md`](seo/inklee-seo-strategy.md) (see [`docs/seo/README.md`](seo/README.md) for the operating model and the ChatGPT-strategy / Claude-implementation split). This document is retained, unchanged in substance, as the **analytical companion**: it holds the detailed live-architecture audit, the Reddit keyword research and decision matrix, the A-vs-B strategy analysis, the prioritization scoring model, the measurement framework, and the validation backlog that the canonical file references. Both files live in the same GitHub repository so both sides work from the same state. Where the two differ on keyword ownership or page architecture, **the canonical file wins**; the known deltas are listed in the canonical file's "Relationship to prior docs" section.

**Status:** Active analytical companion. Created 2026-07-01; recategorized 2026-07-02 (no longer the *canonical* SoT, but still the reference for the deeper analysis and the measurement/validation frameworks).
