...
- **ME-12 (onboarding wizard full pass — added 2026-06-15, founder-flagged):** ◑ IN PROGRESS. End-to-end review/redesign of the onboarding wizard on BOTH surfaces — graphics/illustrations, step functionality, copy, layout, the whole flow. Founder is driving the direction. Routes: web `(artist)/onboarding/*` (welcome → claim-slug → booking → availability → form → done), mobile `app/onboarding/*` (intro → claim → booking → done). Full current-state breakdown of every step/string lives in **`docs/onboarding-redesign-inventory.md`** (the working reference). Pairs with ME-8 (native signup feeds straight into onboarding).
  ...
  - **ME-12 STATUS: web illos + unified copy + the full 5-step wizard port are DONE** (web+mobile tsc + lucide check + onboarding lint clean). Only the optional inline logo-upload-in-done remains. NOT yet committed/deployed; mobile needs a fresh EAS build to verify on-device (no OTA). Memory `onboarding-illustrations` is the live tracker.
