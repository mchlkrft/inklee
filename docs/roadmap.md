...
| Slice 60c (partial) — Mobile pass + onboarding nudges          | 🔄 Partial 2026-05-21                            | Shipped: `md:hidden` card layouts on `/bookings/overview` (Requests + Clients) and `/bookings/waitlist`; StatusActions rendered above details on `<lg` for request-detail; onboarding step-1 label rename; booking-mode picker "Choose this if…" framing. **Still open:** intro slides at wizard start; 375px pass across every onboarding step; OT-08 feature-intro modal copy (Flash, Waitlist, Travel). Commit `8a5ea32`.                                                                              |
...
**Status after 2026-05-21 audit pass (commit `8a5ea32`):** 60a + 60b closed; 60c partially landed; 60d / 60e / 61 still open and now fully parallelizable.
...
| 60c     | Onboarding wizard extension + mobile optimization | 🔄 Partial         | Mobile cards (U5) + actions-at-top on request-detail + onboarding step-1 rename + mode-picker copy shipped. **Remaining:** intro slides before `/onboarding/welcome`, 375px pass across all 5 onboarding steps, OT-08 feature-intro modal copy (Flash, Waitlist, Travel).                                                                           |
| 60d     | Flash feature thorough audit + flow integration   | ⏳ Open            | `docs/flash-ux-audit-slice-60d.md` + flash sits cleanly in the unified booking list                                                                                                                                                                                                                                                                 |
...
**Sequencing now:** 60c-residue, 60d, 60e, 61 all run in parallel — no cross-dependencies remain after 60a + 60b closed.

60c–60d + 61 are audit-and-targeted-fix slices, not redesigns; ship-blocking only if 60d surfaces critical issues. 60e is a build-the-surface slice (artist deposit-settings UI does not exist today) and is ship-blocking only for any environment that still runs Stripe in test mode (the test-mode banner gate).
...
| ---------------------------------- | ----------------------------------------------------------------------------- |
| Public launch                      | OT-02, Slices 60a–60d + 60e + 61, Phase D punch list                          |
