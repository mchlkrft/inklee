...
- ⏳ **THE launch gate is now the payment money path, not UX.** A four-lens audit (2026-06-05, `docs/payment-audit-2026-06-05.md`) found the feature ~90% code-complete but not launch-ready. **Remaining launch gate = Slice 80 Tier 0 code blockers + Tier 3 founder gates + the Phase D live walkthrough:**
  ...
  - **Tier 3 founder gates:** ✅ **G-1 done** (migration 0044 applied 2026-06-05). **G-2 CORE VERIFIED 2026-06-05** in Stripe sandbox (`docs/g2-sandbox-verification.md`): in-app Custom KYC onboarding (no Stripe redirect), EUR deposit with the exact 3% split (artist €194 / Inklee €6, artist = MoR via on_behalf_of), webhook booking-flip, and artist-cancel auto-refund (incl. fee return) all verified against real Stripe data. **G-2 remainder (next session):** client-cancel forfeit (4.2), client-cancel-unpaid intent cancel (4.3), dashboard-refund reconciliation (4.4), multi-currency non-EUR (Phase 3), manual/declined-card edges (Phase 5). **D-d unit-economics decision — launch-gating** (Custom's ~€2/mo per active account + payout fees make low-volume artists net-negative; subscription cover unbuilt, see §4.4) (G-3); D-e counsel sign-off on the Custom model (G-4).
