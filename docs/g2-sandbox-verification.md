...
- [ ] **4.2 Client-cancel a PAID booking** → portal → Cancel request shows the **forfeit warning with the amount** → confirm. A `deposit_forfeited` audit row is written; **no refund** (artist keeps it); booking cancelled.
