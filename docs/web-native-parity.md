...
| Drops, preorders, low-stock alerts (P5c) | product form fields + all 3 public availability gates; alert via the notification plane | ⬜ the native product editor has no drop/preorder/threshold fields yet (tracked in the P5 remainder). The ALERT already reaches native: it uses the existing `system_warning` type, so no new wire value and no new build | ⬜ |
| Discount codes (P5b) | artist editor at `/goods/discounts`; client code field in the portal checkout | ⬜ tracked gap: no native editor yet. The checkout is a CLIENT surface (web by design); the artist-side editor is a real native gap and is listed in the P5 remainder | ⬜ |
