...
| Appointment payment quote + intent (P9 A3) | `server/appointment-payment-quote.ts` (the ONE server-authoritative quote) and `server/appointment-payment-intent.ts` (the Stripe PaymentIntent for a payment request), plus migration `0127` (`payment_requests.payment_intent_id` / `payment_intent_amount_minor`) and the FEE UNIFICATION: the deposit path's `application_fee_amount` now comes from `appointmentApplicationFee` instead of the hardcoded `platformFeeCents`, so one implementation prices both lanes on both paths | ⬜ NOT YET, and the rule for A7 is stronger here than it was for A2. Cores only: no web route, no `/api/mobile/*` route, no screen. **A NATIVE SURFACE MUST NEVER COMPUTE OR SEND AN AMOUNT.** `buildPaymentQuote` takes no amount parameter and is the only producer of one; a mobile route that accepted an amount, or re-derived a total for display, would reintroduce exactly the display-versus-charge divergence spec section 8 exists to prevent, on one platform only. A7 wraps `createPaymentRequestIntentCore` and renders `quote.amountMinor` verbatim. It must also not add an entitlement gate: the quote already refuses server-side, re-derived from the stored row, so a second gate in a route can only disagree. The client-facing payment surface itself is A6 (web) and is a VISITOR surface, so the native question is the artist-side view of a collection in flight, not a payment screen | ⬜ |
