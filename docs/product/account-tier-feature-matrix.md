...
| Booking-form customization (custom colors, templates, branding removal, custom confirmation page, form email templates, custom slug) | `form_custom` (proposed) | preset themes + logo | full | ONE primary form stays the model. **Corrected 2026-07-31 (founder Ruling 1): cover image removed from this row.** It previously listed cover image as part of the Plus-only customization scope; cover image is Free for all artists, not a Plus perk, so it is not part of this boundary (see `plus-commercial-packages.md` §7 and the registry's `form_custom` row) |
