...
| **OQ-7** | Does the 3% deposit fee vary by tier? DECISIONS.md carries both "3% is PROVISIONAL" (2026-06-03) and "the 3% stays as the margin" (D-d, 2026-06-05) | Founder; mechanically one function (`platform-fee.ts`) | Founder | **Keep flat 3% for all deposits, no Plus discount.** The subscription's ~2.70 net barely covers the ~2 EUR/month Connect cost; the ~2.75-per-200-EUR-deposit fee IS the margin (D-d's own logic). Revisit only after G-5 + real margin data exists (see OQ-9). Record the reconciliation in DECISIONS.md so the "provisional" note stops contradicting D-d. |
