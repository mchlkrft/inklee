...
| **B1-B5** bundles scope | founder + eng | ✅ delivered (ruling 18, decision GC6, 2026-08-01) | PAYABLE bundles shipped dark on the standalone shop: first-class `bundle` order line at the bundle price (fee base = bundle price), sale-time composition snapshot (migration 0135) driving fulfilment/restock/deletion-guard, entitlement + visibility + currency gates on the money path. Web-only buyer surface by decision (parity register updated). Mobile checkout n/a (visitor surface); "mobile" in ruling 18 = management parity, already shipped 07-31. |
