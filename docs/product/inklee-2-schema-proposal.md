...
- **Claim conflict** (section 2.1; decision RESOLVED in the Phase 3 claim slice, migration 0079): conflict is a LOCATION-level state. `map_locations.claim_status` gained `claim_conflict`, set by the claim cores when a second pending claim arrives and re-settled on every decision; `location_claims.status` stays the shipped four-value vocabulary. Ownership never changes while contested: approval is admin-only and rejects sibling claims in the same action. Claim INSERTs are service-role-only through the rate-limited action (a client INSERT policy would bypass the rate limit); claimants have SELECT-own with column privileges excluding reviewer identity.
