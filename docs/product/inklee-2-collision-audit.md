...
- **[High] "Studio" currently names two different products.** The roadmap's BM-4.x Studio MVP (25 EUR/month) is multi-artist BOOKING multi-tenancy: central inbox, shared calendar, studio booking page. 2.0's studio owner is a guest spot host with a map page, workspaces, groups, and a blacklist; almost zero feature overlap, same name, same price anchor. The Studio MVP spec even explicitly excludes "marketplace-style discovery". A founder decision must be recorded in `business-model.md`: either 2.0's studio owner IS the new Studio tier definition (rewrite §3.3 and BM-4.x) or it is a separate SKU. Also: Phase 5 Studio Pro already earmarks guest-artist management, time-bounded guest access, and cross-studio demand analytics for a 60 to 80 EUR tier; 2.0 ships versions of these in the base studio role, so the upsell ladder needs conscious reassignment.
- **[High] 2.0 jumps the written phase gates.** All studio work is gated in the docs on "Solo Plus stable 3+ months and 5+ inbound studio inquiries", and the founder principle is "don't enter phase N+1 until N's checklist is met". Building 2.0 now is a deliberate strategy override that no document currently records. Record it as a strategy revision in `business-model.md` and the roadmap during Phase 0, so future sessions do not treat 2.0 work as violating the gates.
- **[Medium] Positioning guardrail:** the feature-scope doc says "not a marketplace or discovery platform". That line is also store-copy source material. Amend the guardrail to scope it to client-facing discovery (which stays true) during the Phase 0 doc reconciliation; keep the honesty rule of not promising studio features until shipped.
