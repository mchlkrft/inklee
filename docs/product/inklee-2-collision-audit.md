...
- **[Medium, half resolved] Shipped 0075 vocabularies need lockstep extension.** Two CHECK constraints already live on prod: `map_reports.reason` (wrong_location, fake_studio, spam, scam, behavior, other) and `location_claims.status` (pending, approved, rejected, revoked). The report context categories replace the first with the ten-value vocabulary in Phase 7 (still open). The claim conflict workflow RESOLVED the second differently (2026-07-18, migration 0079): `location_claims.status` stays untouched; the conflict state landed on `map_locations.claim_status` as `claim_conflict`, moved in lockstep with the shared `MAP_CLAIM_STATUSES`.
...
## 15. Seeding tool risks (added 2026-07-18)

Full context in `inklee-2-map-seeding-tool.md`.

- **[High] The Brave lane has no structural cost guarantee.** A payment card is on file with Brave; the usage ledger (hard stops at 60/day and 900/month, blocked requests logged, refuse-before-request ordering) is the only wall between the tool and real charges. A guardrail bug, a Brave price change, or ledger-table unavailability handled wrongly could cost money. Founder accepted this residual risk 2026-07-18 with corrected facts in hand. Mitigations: the lane fails closed when the ledger is unreadable, the request is counted before it is sent, and the lane is off entirely without MAP_SEED_BRAVE_SEARCH_KEY.
- **[Medium] Search API landscape rot.** Google CSE is closed to new customers (dead January 2027), Bing retired, Brave metered. The Brave lane can disappear or reprice at any time; the tool must stay fully functional on Overture plus manual lanes alone (it does; the provider vocabulary keeps the slot).
- **[Medium] Overture license mixing.** The places theme is CDLA-Permissive-2.0 overall, but Foursquare-sourced rows are Apache 2.0. Both are permissive; the per-candidate attribution note plus the retained candidate row (provenance survives conversion) is the compliance surface. Q20 covers the public-page scenario.
- **[Medium] Candidate table growth and staleness.** Leads are uncapped by design, so the table grows monotonically and rejected leads linger. Not wired into any retention purge yet; revisit when the first real seeding wave shows actual volumes.
- **[Low] Overture schema drift.** The places categories column is mid-migration (categories deprecated toward basic_category/taxonomy, removal announced for September 2026). The extraction script tries both shapes; expect to touch it once per Overture schema change.
- **[Low] Conversion double-write bookkeeping.** Conversion creates the map entry through the shared pipeline, then marks the candidate converted. The second write can fail after the first succeeded; the action surfaces this explicitly (entry created, bookkeeping failed) instead of pretending the conversion failed, and the candidate can be re-marked.
