...
- **Post-flip the collector will track SIGNED-IN artists on `/map`** (medium). The S4 carve-out is plane-blind: it removes `/map` from the private prefixes for everyone, so artist workspace visits start flowing into the anonymous acquisition collector. Recommend gating the `trackPublicEvent` calls on `isPublic` so the acquisition numbers stay acquisition numbers. Worth doing before the soak reads anything into map traffic.
