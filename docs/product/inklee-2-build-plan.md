...
Status: phased build structure for the guest spot map and studio network track, written 2026-07-17. The original "no implementation has started" framing is long obsolete: Phases 1 to 4 have largely shipped (per-phase SHIPPED notes below), the map is live in production for signed-in artists, and the public shell is postponed pending the future-scope pass that landed 2026-07-27. See the status correction block below for the current picture. Scope and locked decisions live in `docs/product/inklee-2-guestspot-map-scope.md`; collision details in `docs/product/inklee-2-collision-audit.md`; the map's long-term direction and planning horizons in `docs/product/inklee-2-map-growth-strategy.md`.

## Status correction (2026-07-27)

The per-phase SHIPPED notes below were written as slices landed and several summary lines have gone stale since. Corrections verified against code on 2026-07-27 (superseded lines below also carry inline notes):

- **The map is LIVE in production for signed-in artists** (`NEXT_PUBLIC_TATTOO_MAP` flipped ON 2026-07-19; 16 countries, ~71k approved studios). The immersive full-viewport shell is THE discovery surface; its `NEXT_PUBLIC_MAP_IMMERSIVE_SHELL` flag was retired 2026-07-25 after the prod soak and the legacy boxed discovery client was deleted (`75d07eb`).
- **Mobile is no longer "not started".** The native tattoo map (discover screen on the same shared map core, detail sheet, watch, and the guest spot request form) plus 7 `/api/mobile/map/*` routes shipped 2026-07-26 (`10f1b9d`, `tattoo_map` capability kill switch). Native remains submit-only for guest spots (no native request tracking, studio inbox, corrections, claims, or map presence settings); the parity register `docs/web-native-parity.md` is the tracked view.
- **Q9 notification wiring SHIPPED** (Q9 was resolved 2026-07-19: feed + push + email mirroring the booking pattern). Six guest spot notification kinds fan out via `notifyGuestSpotEvent`; blacklisted requests notify nobody. The Phase 4 line saying Q9 is "still an open founder decision" is superseded.
- **Q16 default flipped and shipped.** Guest artists are named by default with a per-artist opt-out (`profiles.guest_naming_opt_out`, migration 0102, founder decision 2026-07-22). The Phase 4 timeline paragraph describing always-anonymized current/upcoming entries is superseded.
- **Temporary studio signals SHIPPED** (migration 0092, Q7 display decisions applied: 8-type vocabulary, 14-day expiry, 1 per owner per month, watcher in-app notification only).
- **Trust surface partially shipped since:** `possibly_closed` (0103) with correction-driven admin action, `last_confirmed_at` written on claim approval AND on every owner profile update (both clear `possibly_closed`; no non-owner freshness writer exists), the DSA `moderation_statements` writer (2026-07-23), the map corrections loop (0098), and the styles-represented display read path on claimed published detail surfaces (the style FILTER path remains unbuilt).
- **Public-shell compliance surfaces:** `/data-attribution` (licences, Foursquare NOTICE, GDPR Art. 14 disclosure) and the marketing integration are built and DARK behind `publicMapEnabled()`; the Art. 21 delisting category on `/legal/report` is already LIVE (the report form is a public 1.x surface, ungated); the DSA procedure is at v2. Provenance migration 0111 APPLIED 2026-07-27 with the shell slices (go-live plan S3): columns + index verified live, backfill covers every row, and the Q17 Brave-title overwrite touched the measured 10 terminal rows.
- **`MAP_PINS_V2` (0101) validated bad and must stay OFF** (drops pins on wide viewports; v1 is already index-backed). The v2 functions are dormant.
- **Known truthfulness gaps to carry into future slices:** claimed and unclaimed pins render identically on the map canvas (distinction is text-only); claimed studio photos/logo are publish requirements that no visitor surface renders; `profiles.travel_map_consent` is collected by `/settings/map` but no read path consumes it (the promised trip-cities display does not exist); seeded `private_studio` entries render at true coordinates (the approximate-offset protection only applies to owner studios).

Future work is organized in the "Future map growth track" section at the end of this file; it never modifies the historical phase records above it.
...
- Extension (2026-07-18): the map seeding tool, SHIPPED 2026-07-18 (migration 0082 + `/admin/map/seeding`; full SoT `inklee-2-map-seeding-tool.md`). A strict zero-expense lead collector, not an importer: seed areas (planning entities with live bucket capacity), a candidate queue fed by Overture Maps (local DuckDB extraction script + preview-then-commit import lane, CDLA-Permissive-2.0), Brave Search leads under hard ledger guardrails (60/day, 900/month, blocked requests logged; founder-accepted card-on-file risk 2026-07-18 after Google CSE closed to new customers), and manual Instagram entry; artist suggestions are planned as a follow-on slice. Candidates are uncapped leads with provenance and duplicate annotations (reusing the 0077 classifier); conversion routes through the existing create form and pipeline, so the density cap and duplicate warn-and-confirm fire exactly once, in one place. Q2 provisionally answered by the founder; new open questions Q17 to Q20.
...
Status 2026-07-18: request core slice SHIPPED (migration 0080, behind the flag). `@inklee/shared/guest-spots` carries both ported FSMs (14-state request superset with the streamlined v1 transition additions: direct decisions from submitted, declined reachable from every reviewable state, a propose self-loop for revised suggestions, accepted as the retryable materialization intermediate) plus the request validation and the locked-leg message. Migration 0080: `guest_spot_requests` (one-open-request partial unique per artist+studio), `guest_spot_proposals` (separate rows, supersede semantics), `guest_spot_stays` (UNIQUE per request), `guest_spot_private_notes` (party-private, owner-authored), `studio_blacklists` (owner-SELECT only, artist can never observe it), the `trip_legs` linkage columns (`origin`, `guest_spot_stay_id` + one-leg-per-stay unique), and DB-level guard triggers making guest-spot legs and their trips read-only for client roles (the app-layer guards in the travel actions and both mobile routes are the friendly layer on top). Server core: submit (published+accepting studios only, 5/24h rate limit), Accept / Pass / Suggest dates (revisable), private notes, artist withdraw + take-suggested-dates, cancel stay from either side (removes only the materialized leg, keeps artist-added stops, trip goes only when empty); acceptance materializes trip + leg idempotently (stay dates win on retry, concurrent double-accept compensated). Surfaces: map detail CTA + request form, `/travel/requests` (+ detail with proposal accept/withdraw/cancel), `/studio/requests` inbox (quiet-hold blocked section collapsed; blocked requests narrow to Pass on the detail) + stays with cancel, cockpit guest spot section. 29-check local RLS battery green; a 2-lens adversarial review produced 16 findings, all fixed (headliners: the transition pre-validation veto that broke Pass entirely, the missing blacklist table, double-materialization orphans, retry-with-wrong-dates after proposal acceptance, invitation_only treated as requestable). Q9 notification wiring deliberately NOT included at this point. *(Superseded: Q9 was resolved 2026-07-19 and the wiring shipped since; every item in the remaining list below except full mobile parity also shipped in the follow-on slices recorded in the paragraphs after this one. Native remains submit-only. See the status correction block.)* Remaining in Phase 4 as of this 2026-07-18 note: house rules builder, welcome pack, guest artist timeline, artist passport, pending-request calendar markers, confirmation entry/terms snapshot use, the private bucket for attachments, stay lifecycle transitions (active/completed), and mobile.

Welcome pack attachments SHIPPED 2026-07-18 (migration 0086; the Phase 4 private bucket): the `welcome-pack-files` bucket (studio-keyed, no storage policies, service-role plus short signed URLs, the studio-media posture) with a metadata table (owner-SELECT RLS). Owners attach up to 5 files (PDF or image, 4 MB) in the welcome pack editor; only artists with a confirmed or active stay can open them, through the same gate as the pack text, and a files-only pack renders. Review fixes: exact-count cap check, input reset on failed upload, path-guard tests. Support-ticket attachments (the other FU-20 consumer) stay a separate follow-on.

Confirmation terms snapshot SHIPPED 2026-07-18 (no migration; the 0080 terms_snapshot column gets its writer): at first materialization the stay captures what was agreed (dates, studio name, the house rules as they stood, captured-at stamp) through the upsert insert path, so retries and later rule edits never rewrite history. Both request pages show an "Agreed at confirmation" block: the artist gets the frozen rules collapsible, the studio gets the capture note. This is the lightweight structured confirmation entry from the plan; no PDF, per the locked scope.

Pending-request calendar markers SHIPPED 2026-07-18 (no migration, behind the flag): the artist's 1.x calendar shows each open guest spot request as one dashed chip on the requested range's first day (label "StudioName?", links to the request; also listed in the busy-day popover; legend entry appears only when markers exist). Client bookings are never blocked, by construction (markers only). Confirmed guest spots already render through their materialized trip legs, so no extra wiring was needed there. Flag off leaves the 1.x calendar byte-identical.

Guest artist timeline slice SHIPPED 2026-07-18 (migration 0085, behind the flag; Q16 applied): `studio_profiles.show_guest_timeline` (default off, checkbox in the profile editor) puts a Now / Coming up / Past guest list on the studio's map page, read from stays (cancelled and no-show never appear). Privacy per Q16 plus one deliberate narrowing: current and upcoming entries are ALWAYS anonymized ("A guest artist" + dates); past entries carry a name and link only for passport_public artists, because the passport toggle's consent covers completed history, not future whereabouts. Naming current/upcoming guests would need its own consent decision (founder call, open). The passport toggle copy now discloses the past-list naming. *(Superseded 2026-07-22: the founder updated Q16 to named-by-default with a per-artist opt-out, shipped as `profiles.guest_naming_opt_out` in migration 0102 and decoupled from `passport_public`. The timeline now names guests across current/upcoming/past unless the artist opted out.)*

Artist passport slice SHIPPED 2026-07-18 (migration 0084, behind the flag): the read model over completed stays, private by default. `profiles.passport_public` (shipped ungranted in 0076 per the 0074 enumeration rule) got its write site: a toggle on /settings/map, with the 0084 column grant and the seed.sql mirror. `getArtistPassport` returns the completed count plus the most recent studios (deduped by studio) only when the artist opted in; the one surface is the studio's request detail page (an applicant's passport as positive evidence), hidden entirely at zero completed stays so a new artist's toggle never reads as a negative signal. Review clean beyond two display fixes.

Stay lifecycle slice SHIPPED 2026-07-18 (no migration): a date-driven, idempotent sweep in the daily cleanup cron. Confirmed stays activate on their start date, active stays complete the morning after their end date, and each completed stay's request follows (confirmed to completed). The request follow-up is state-driven (derived from completed stays, not from the same-run delta) so a mid-sweep crash self-heals on the next run; the sweep runs before the cron's early returns so quiet days never skip it; all steps log failures. Runs regardless of the UI flag (data hygiene; empty tables no-op). This makes completed stays real data, unblocking the guest artist timeline (Q16 pending) and the artist passport.

Welcome pack slice SHIPPED 2026-07-18 (migration 0083, behind the flag): `studio_welcome_packs` (one per studio, six structured text sections: access, wifi, emergency contact, supply shops, promotion, local notes; owner-SELECT RLS, service-role writes; `include_house_rules` reserved for the Q9 send surface). Owner editing in /studio/edit below house rules. Interaction plane, strictly: the pack renders ONLY on the request page of an artist holding a confirmed or active stay at the studio (double-gated: page condition + a stay re-check in the guest read core); cancelled, completed, and no-show stays never unlock it, and the table itself is invisible to artists via PostgREST. Not sent anywhere: per the open Q9 decision the pack appears after confirmation, it is not pushed. Attachments wait for the private bucket. 2-lens review: one finding (a dead include-house-rules control) fixed by dropping the control until Q9 gives it a consumer.

House rules slice SHIPPED 2026-07-18 (migration 0081, behind the flag): `studio_house_rules` (typed keys only in v1, the ten build-plan categories, one row per key per studio, owner-SELECT RLS, service-role writes). Shared vocabulary + labels + 500-char content validation + canonical ordering in `@inklee/shared/studio-profile`; the completeness score gained an optional "House rules set" item (never a publish blocker, per the extension's own rule). Owner editing lives in /studio/edit (add-rule chips + per-rule textareas, loss-free diff replace on the categories pattern); display renders for published studios only, on the map detail page and on the requesting artist's request detail (approval is where rules start mattering). Draft and suspended studios never leak rules (publication re-checked at read time). 6-check RLS battery green; 2-lens review found no defects (one type-guard hardening applied). The welcome pack (next slice) reuses these rows as its first content block.
...
*(Update 2026-07-27: the first native pickup happened. The native tattoo map, detail sheet, watch, and the guest spot request form shipped 2026-07-26 on the shared map core (`10f1b9d`), with the `tattoo_map` capability registered as the kill switch, exactly per the constraints below. Still native-absent: guest spot request tracking and the studio inbox, corrections and claims, and the map presence settings. Every further native-affecting change updates `docs/web-native-parity.md` in the same change, per the founder rule.)*
...
## Future map growth track (added 2026-07-27)

The map future-scope pass (`docs/product/inklee-2-map-growth-strategy.md`, the product source of truth for this track) organizes everything beyond the numbered phases above into four horizons. This section is the build-plan view of that strategy: what gates what, and what may NOT be treated as scheduled. The historical phases above are not renumbered; Phases 5 to 9 (workspaces, groups, reputation, shops, audit and merge) remain their own founder-sequenced track and are unaffected by this section.

Rules for this track:

- Nothing in Horizons 2 to 4 is implementable from this document alone. Each capability needs the dependency mapping from the growth strategy §13 (canonical entity, data, consent, moderation, analytics, notifications, mobile parity row, legal, SEO, founder decision, flag, kill switch, rollback, horizon) before it becomes a slice.
- Speculative features never enter the public-launch critical path, and validation candidates never enter an active phase without evidence.
- Open questions Q21 to Q29 (`inklee-2-open-questions.md`) are never resolved inside a slice.

### Horizon 1: trust and public utility (contains the public launch critical path)

Execution plan: `docs/product/inklee-2-public-map-golive-plan.md` (written 2026-07-27; slices S1 to S6, go/no-go gate, flip and rollback procedure, soak plan).

The public shell and the launch prerequisites. The authoritative critical-path list lives in roadmap §6.6 (anonymous branch on the four `/api/map/*` routes, public chrome outside `(artist)`, explicit `robots: { index: false, follow: true }`, plane-1 caching + per-IP rate limiting, the `/map` carve-out in the analytics collector's private prefixes plus registered map events, enforcement of the declared-but-unread `MapCapabilities` fields, migration 0111, artists-in-town counts-only floored at 3 pending Q21, the `/data-attribution` + credit surfaces going live with the flip, and the two 2026-07-27 review items below); this section points at it and does not redefine it.

The scope pass added exactly two items to that critical path (both are review/correction items that gate the flip):

- Review the seeded `private_studio` display-coordinate handling (the approximate-offset protection currently applies only to owner studios; the scope's "a private studio cannot be shown at its exact map position" rule is not yet honored for seeds). *(Founder D3, 2026-07-27: remediate as recommended, deterministic offset plus address nulling; go-live plan slice S3.)*
- Correct the `/map` robots meta: the authenticated layout emits `noindex, nofollow` today while the SEO strategy specifies `noindex, follow` for the public surface.

Founder decisions 2026-07-27 (DECISIONS.md row) reshaped the launch scope: **D1** puts the claimed `/studios/{slug}` entity pages IN v1 (go-live plan slice S2b: slug system, gated route, JSON-LD, generated studio sitemap segment, claimed media rendering), and **D2** postpones the artists-in-town layer entirely on the public plane (not even counts; returns only through Q21).

Horizon 1 trust work that ACCOMPANIES the flip (may land before or after it; does not gate go-live):

- Visible claimed vs unclaimed pin distinction on the canvas (today text-only).
- Account walls that preserve the visitor's intended action through signup (the plan's v1 ships the return-to basics).
- The guest-spot-available map filter (a Horizon 1 candidate; data exists on `studio_profiles.guest_spot_status`, needs the RPC filter path).
- In-map reuse of the S2b claimed-media path on the detail panel (the entity page itself renders claimed media in v1 per D1).

### Horizon 2: retention and professional planning

Notification preferences first (Q22; no per-type preference surface exists and alert expansion is blocked on it), then: watch alerts beyond the shipped signal notification, save cities, saved searches, studio lists, the trip workspace (shortlists, approximate dates, comparison), opportunity cards, artist presence controls including resolving the `travel_map_consent` read path (ship the trip-cities display or retire the toggle copy), the style FILTER read path (a display read path is already shipped: styles-represented chips on claimed published detail surfaces; what is missing is filtering the map by style), and owner-declared decision data (minimum stay, equipment, languages).

### Horizon 3: marketplace liquidity without self-booking

Studio request board as an evolution of the shipped temporary-signal system (Q24, never a parallel post type), mutual interest and structured introductions, city activation tooling, city pulse (in-map, non-indexable), shareable tours and collections (Q23), embeds (Q25), studio map analytics (Q26). Request-based and artist-controlled throughout; never unrestricted marketplace booking.

### Horizon 4: validated network and monetization expansion

Advanced matching with the transparency safeguards, advanced alerts, team and roster tools (including the deferred residency roster, founder 2026-07-22), demand insight, promotion products (Q27 + business-model work, no pricing invented), validated city SEO pages and style pages (each through the SEO strategy's own gates). Every Horizon 4 item requires real product evidence and separate validation.
