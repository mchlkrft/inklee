...
export const PLUS_CAPABILITY_REGISTRY: PlusCapability[] = [
  ...
  },
  {
    ...
    plusBehavior:
      "Image galleries today (upload OR server-side import from a URL, layout grid/carousel); the entitlement is deliberately named for the FAMILY, not just galleries, so future rich sections (video, testimonials) land on the SAME key rather than minting a new one each time",
    legacyBehavior:
